...
const links = [
  ...
  { href: "/editor", label: "Generador de Flujos", icon: Code2 },
  ...
  return (
    ...
    >
      <div className="px-5 py-4 flex items-center justify-between border-b border-white/10">
        ...
        <button onClick={onClose} className="md:hidden text-white/70 hover:text-white" aria-label="Cerrar">
          ✕
        </button>
      </div>
        ...
        {links.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              />
              <Icon size={18} className={`${active ? "text-cyan-300" : "text-white/70 group-hover:text-white"}`} />
              <span>{label}</span>
            </Link>
          );
        })}
